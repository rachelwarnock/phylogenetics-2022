...
For this exercise you can create a folder called `exercise 4` or something. The create two sub-directories, `data` for your nexus file and another `scripts` for your code. For this tutorial we'll create t least three scripts:

  1. `main.Rev` for reading the data, setting up the the tree model and MCMC settings 
  2. `JC.Rev` and `GTR.Rev` for alternative subsitution models.
...
```{r data, eval=FALSE}
data <- readDiscreteCharacterData("data/primates-data.nex")
data
```

Next we need to define some varibles that will come in handy later for setting up our model.
...
Next we'll define a set of variables for setting up our MCMC. Note that we use `=` instead of `<-` because these variables are not part of the model. The `moves` vector is for storing our MCMC moves and the `monitors` vector is for storing the recording output. 
...
For setting up regular Bayesian tree inference we need to specificy two model components: 

  1. we need a prior the topology and branch lengths 
...
```{r tree_prior2, eval=FALSE, echo=FALSE}
...
#TODO: you have to make the primate data still works with exercise 3 (STANDARD -> DNA)
...
This basically tells the program to calculate the likelihood for this data using the substitution model specified using the function arguments (i.e. calculate the likelihood using the JC model represented by the Q matrix). We don't need to add any additional moves here because we're not estimating any extra parameters.

It might seem a bit overkill to have such a small number of code line in a separate script but as the models become more complex you'll see why this becomes useful. 
...
```{r monitors, eval=FALSE, echo=FALSE}
# parameter printed to file
...
Let's increase the chain length to 10000 and rerun the MCMC. This might take up to 15 minutes - while you're waiting for this run, you could move on and set up the GTR substitution model. 

You're already getting a flavour of how long MCMC analyses can take. It's very common to have to run empirical analyses for millions+ generations and it can take several days.
...
Actually, if wanted to use this output for publication, we'd still want to run the chain for longer because some parameters haven't mixed well but let's move one.
...
Under this model both rates of change between states and state frequencies are allowed to vary. This makes a little bit more complicated to set up - but not by much!
...
```{r Q3, eval=FALSE, echo=FALSE}
er ~ dnDirichlet(alpha1)
freq ~ dnDirichlet(alpha2)
```

`alpha1` and `alpha2` are the parameters of the Dirichlet distribution. These are used to specify two uniform Dirichlet distributions. 
These are stochastic variables because we want to estimate the relative rates and state frequencies during the analysis.
We then set up the Q matrix as a deterministic variables, which will change through the MCMC as `er` and `freq` are updated.
...
The rest of the set up as the same as before.
...
Back in your `main.Rev` script, switch out the file used to load subsitution model.
...
>What differences do you notice between the runs? Are there any additional parameters? Are these any differences between equivalent parameters? 
