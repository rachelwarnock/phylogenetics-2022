...
In this exercise we'll estimate of a time tree of bears under the fossilised birth-death (FBD) process. This time the data we'll use is a [morphological matrix](data/bears_morphology.nex) for 16 living and fossil bears. We'll also use [age information](bears_taxa.tsv) associated with all taxa. Download these files and check out the contents. 
...
moves.append( mvFNPR(tree, tune = true, weight = 15.0) )
moves.append( mvCollapseExpandFossilBranch(tree, origin_time, tune = true, weight = 6.0) )
moves.append( mvNodeTimeSlideUniform(tree, tune = true, weight = 40.0) )
moves.append( mvRootTimeSlideUniform(tree, origin_time, tune = true, weight = 5.0) )
```
...
moves.append( mvFossilTimeSlideUniform(tree, origin_time, tune = true, weight = 5.0) )
```
...
For simplicity we'll a strict clock model to describe the rate of character change over time, assuming the rate is constant over time and across all branches in our tree. **Hint**: you can use your clock script from the [previous exercise](exercise-04.html#strict).
